...
public class MapGen : IMapGen
{
    ...
    public WorldMap GenerateWorld(int width, int height, int countries)
    {
        ...
        for (int i = 0; i < countries; ++i) {
            ...
            capital.x = rng.NextDouble() * width;
            capital.y = rowHeight * i + rng.NextDouble() * rowHeight;
            if (grid[(int) capital.y, (int) capital.x] != MapCell.WATER) {
                --i;
                continue;
            }
            grid[(int) capital.y, (int) capital.x] = MapCell.CapitalCity(i);
        }
